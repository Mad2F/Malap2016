...
def test_mlp(
    ...
    while epoch_counter < n_epochs:
        ...
        validation_losses = [test_model(i) for i in range(int(n_valid_batches))]
        ...
        results_file.write(bytes("{0}\n".format(this_validation_errors), 'UTF-8'))
        results_file.flush()

        new_learning_rate = decay_learning_rate()

    end_time = time.clock()
    s = 'The code for file ' + os.path.split(__file__)[1] + ' ran for %.2fm' % ((end_time - start_time) / 60.)
